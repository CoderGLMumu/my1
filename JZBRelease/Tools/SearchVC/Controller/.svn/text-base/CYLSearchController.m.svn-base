...
- (UIView *)searchBackgroundView
{
    if (_searchBackgroundView == nil) {
        ...
        _searchBackgroundView.backgroundColor = [UIColor redColor];
...
{
    //11 20
    UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 1, 35)];
    UIImageView *backImageView = [UIImageView createImageViewWithFrame:CGRectMake(0, (35-20)/2, 1, 20) ImageName:@""];
...
//    [UIApplication sharedApplication].keyWindow.rootViewController = delegate.navigationController;
...
- (void)reloadViewLayouts
{
    if (self.isShowQuestions) {
        ...
        self.tableView.frame = CGRectMake(self.tableView.frame.origin.x,
                                          self.tableView.frame.origin.y,
                                          self.tableView.frame.size.width,
                                          MIN(self.searchHistories.count * 44 + (self.searchHistories.count > 0 ? kHeightForFooterInSection : 0), [UIScreen mainScreen].bounds.size.height));
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (self.isShowQuestions) {
        ...
        if (self.questionDataSource.count) {
...
//        cell.textLabel.font = [UIFont systemFontOfSize:14];
        cell.user = self.questionDataSource[indexPath.row];
        return cell;
...
//        [tableView deselectRowAtIndexPath:indexPath animated:YES];
        Users *user = self.questionDataSource[indexPath.row];
