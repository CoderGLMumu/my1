...
- (void)MessageBanSpeak:(NSNotification *)note
{
    
    [self BanSpeak:note.userInfo];
...
/** 高林 禁言 */
- (void)BanSpeak:(NSDictionary *)dict
{
    if (![_conversation.conversationId isEqualToString:dict[@"groupid"]]) return;
    
    UserInfo *userInfo = [[LoginVM getInstance]readLocal];
    
    if (![userInfo._id isEqualToString:dict[@"uid"]]) return;
...
//        [self.recordView removeFromSuperview];
        ...
        markView.frame = CGRectMake(0, 0, _chatToolbar.glw_width, _chatToolbar.glh_height);
        markView.backgroundColor = [UIColor hx_colorWithHexRGBAString:@"ffffff"];
        [_chatToolbar addSubview:markView];
        
        UILabel *markTitleLabel = [UILabel new];
        [markView addSubview:markTitleLabel];
        markTitleLabel.text = @"现在是听讲模式,需要发言请联系管理员";
        [markTitleLabel sizeToFit];
        
        markTitleLabel.center = markView.center;
...
- (void)viewDidLoad {
    ...
    /** 高林 禁言功能 黑名单 */
//    [self gaolinjingyangongneng];

    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(MessageBanSpeak:) name:@"MessageBanSpeak" object:nil];
...
//    [self.view addSubview:backView];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(didBecomeActive)
                                               ...
                                               object:nil];
    /** 高林 chat 聊天气泡 */
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[[UIImage imageNamed:@"EaseUIResource.bundle/chat_sender_bg"] imageWithTintColor:[UIColor hx_colorWithHexRGBAString:@"ffffff"]] stretchableImageWithLeftCapWidth:5 topCapHeight:35]];
...
- (void)dealloc
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"MessageBanSpeak" object:nil];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSDictionary *noteDict = [udefaults objectForKey:@"HXforbid"];
    
    [self BanSpeak:noteDict];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        timeCell.backgroundColor = [UIColor hx_colorWithHexRGBAString:@"eeeeee"];
        return timeCell;
    }
    else{
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(messageViewController:cellForMessageModel:)]) {
            ...
            if (cell) {
                ...
                cell.backgroundColor = [UIColor hx_colorWithHexRGBAString:@"eeeeee"];
                return cell;
            }
        }
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(isEmotionMessageFormessageViewController:messageModel:)]) {
            ...
            if (flag) {
                ...
                sendCell.backgroundColor = [UIColor hx_colorWithHexRGBAString:@"eeeeee"];
                return sendCell;
            }
        }
