...
//    }
    ...
    if (imageAry.count > itemIndex) {
        [cell.imageView setImage:[imageAry objectAtIndex:itemIndex]];
    }else{
        if ([imagePath rangeOfString:@"/"].length > 0) {
            dispatch_sync(dispatch_queue_create("imageQueue", nil), ^{
                UIImage *image = [LocalDataRW getImageWithDirectory:Directory_XB RetalivePath:[[ValuesFromXML getValueWithName:@"Images_Absolute_Address" WithKind:XMLTypeNetPort] stringByAppendingPathComponent:imagePath]];
                image = [ZJBHelp handleImage:image withSize:self.frame.size withFromStudy:YES];
