...
- (void) SetMoiveSource:(NSURL*)url
{
    mSourceURL = [url copy];
    if (!url) {
//        UILabel *backGLabel = [UILabel new];
//        backGLabel.text = @"网络错误或者视频链接已经失效,如需帮助请联系我们";
//        backGLabel.font = [UIFont systemFontOfSize:21];
//        [self.view addSubview:backGLabel];
//        backGLabel.frame = CGRectMake(0, 0, GLScreenW, GLScreenH);
//        backGLabel.numberOfLines = 0;
//        backGLabel.textAlignment = NSTextAlignmentCenter;
//        backGLabel.textColor = [UIColor whiteColor];
    }
}
...
- (void) setupControls
{
    ...
    mPlayerView.frame = self.videoFrame;
...
//    [bottomBar addSubview:seekBackwardButton];
    ...
    self.activityBackgroundView1 = activityBackgroundView;
...
//    CGFloat width = self.videoFrame.size.width;
//    CGFloat height = self.videoFrame.size.height;
...
-(void)UpdatePrg:(NSTimer *)timer{
    ...
    double liveCurrentTime = mPlayer.currentPosition / 1000 / 60 * 60;
    
    double minutesElapsed = floor(fmod(liveCurrentTime/ 60.0,60.0));
    double secondsElapsed = floor(fmod(liveCurrentTime,60.0));
    double hourElapsed = floor(liveCurrentTime/ 3600.0);
    
    /**  回调设置后的已经播放时间文本 */
    if (self.callBack) {
        self.callBack([NSString stringWithFormat:@"%02.0f:%02.0f:%02.0f", hourElapsed,minutesElapsed, secondsElapsed],VideoActionTypeNULL);
    }
...
//        seekBackwardButton.hidden = YES;
    ...
    if (self.callBack) {
        self.callBack(@"PreparedPlay",VideoActionTypePreparedPlay);
    }
...
- (void)OnVideoError:(NSNotification *)notification {
    ...
    if (self.callBack) {
        self.callBack(@"error",VideoActionTypeVideoError);
    }
