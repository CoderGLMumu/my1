...
- (void)tableViewCell:(TableViewCell *)cell didClickedImageWithCellLayout:(CellLayout *)layout atIndex:(NSInteger)index {
    ...
    for (NSInteger i = 0; i < layout.imagePostionArray.count; i ++) {
        ...
        rect1 = CGRectMake(rect1.origin.x, rect1.origin.y + 44, rect1.size.width, rect1.size.height);
        ...
        if (0 == indexPath.row) {
            ...
            if (appDelegate.audioSwitch) {
                ...
                if (evaluateModel.audio && evaluateModel.audio.length > 0) {
                    return layout.cellHeight + 44 + 44 + 10 + 48;
                }
            }
            return layout.cellHeight + 44 + 44 + 10;
        } 
    }
    return 0;
}
            ...
            if (1 == state) {
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    if ([self.questionModel.is_like intValue] == 1) {
                        ...
                        [SVProgressHUD showInfoWithStatus:@"亲，您太喜欢学习了，已经成功关注了该话题，请记得及时关注话题动态，并积极参与讨论喔！"];
    ...
    sendAndGet.returnModel = ^(GetValueObject *obj,int state){
        if (!obj) {
            ...
            if (1 == state) {
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    if ([evaluate.is_like intValue] == 1) {
                        [SVProgressHUD showInfoWithStatus:@"分享是一种功德，点赞是一种美德！您的参与棒棒哒！"];
...
- (void)showShareActionSheet:(UIView *)view
{
    ...
    NSString *shareUrl_str = [[ValuesFromXML getValueWithName:@"Sever_Absolute_Address" WithKind:XMLTypeNetPort] stringByAppendingFormat:@"%@", [NSString stringWithFormat:@"/Share/Question/info/id/%@.html",self.questionModel.question_id]];
