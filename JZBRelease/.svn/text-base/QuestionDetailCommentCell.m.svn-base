...
- (void)setCellLayout:(CellLayout *)cellLayout {
    ...
    if (!self.infoView) {
        self.infoView = [SimpleInfoView getInfoShowViewWithData:_cellLayout.questionsModel.user WithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 44)];
    ...
    [self.asyncDisplayView setFrame:CGRectMake(0, 44, SCREEN_WIDTH, cellLayout.cellHeight)];
    ...
    if (!self.tabItemBarView) {
        ...
        self.tabItemBarView = [CommentTabItemBarView getCommentTabItemBarViewWithFrame:CGRectMake(0, cellLayout.cellHeight + 44, SCREEN_WIDTH, 44) IsQuestionOwner:NO];
    ...
    if (self.audioPlayView && !self.audioPlayView.hidden) {
        [self.tabItemBarView setFrame:CGRectMake(0, self.audioPlayView.frame.origin.y + self.audioPlayView.frame.size.height + 18, SCREEN_WIDTH, 44)];
    }else{
        [self.tabItemBarView setFrame:CGRectMake(0, cellLayout.cellHeight + 44, SCREEN_WIDTH, 44)];
    }
