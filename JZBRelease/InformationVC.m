...
- (void)createData{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (self.industryAry && self.industryAry.count > 0) {
            [self downloadData];
        }else{
            [self downloadTag];
        }
        ...
        if ([model.position isEqualToString:@"8"]) {
            if (model.cover && model.cover.length > 0 && (range0.length == 0) && (range1.length == 0)) {
                if (fabs((double)MAX(Main_Screen_Width, Main_Screen_Height) - (double)568) < DBL_EPSILON) {
                    model.cellHeight = [self returnHeighWithText:model.title] + 300 * 150 / 355 + 10 + 8  + 38;
                }else if (fabs((double)MAX(Main_Screen_Width, Main_Screen_Height) - (double)736) < DBL_EPSILON){
                    model.cellHeight = [self returnHeighWithText:model.title] + 394 * 150 / 355 + 10 + 8 + 38;
                    
                }else if (fabs((double)MAX(Main_Screen_Width, Main_Screen_Height) - (double)667) < DBL_EPSILON){
                    model.cellHeight = [self returnHeighWithText:model.title] + 150 + 10 + 8  + 38;
                }
            }else{
            ...
            if (model.cover && model.cover.length > 0 && (range0.length == 0) && (range1.length == 0)) {
                if (fabs((double)MAX(Main_Screen_Width, Main_Screen_Height) - (double)568) < DBL_EPSILON) {
                    model.cellHeight =  320.0 / 375.0 * 77 + 10 + 15;
                }else if (fabs((double)MAX(Main_Screen_Width, Main_Screen_Height) - (double)736) < DBL_EPSILON){
                    model.cellHeight = 414.0 / 375 * 77 + 10 + 15;
                    
                }else if (fabs((double)MAX(Main_Screen_Width, Main_Screen_Height) - (double)667) < DBL_EPSILON){
                    model.cellHeight = 77 + 10 + 15;
                }
