        ...
        if (![arr isKindOfClass:[NSNull class]]) {
            for (NSDictionary *dictT in arr) {
                ...
                if ([dictT[@"is_identity"] isEqualToString:@"1"]) {
                    ;
                    [self.touxiangImageView sd_setImageWithURL:[NSURL URLWithString:[AddHostToLoadPIC AddHostToLoadPICWithString:dictT[@"avatar"]]] placeholderImage:[UIImage imageNamed:@"grzx_contacts_qun"]];
                }
            }
