...
@interface ZBYGCoursTimeVC ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate>{
...
@implementation ZBYGCoursTimeVC
...
- (void)createTableView{
    ...
    tableView.backgroundColor = [UIColor hx_colorWithHexRGBAString:@"ffffff"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BigViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    
    if (!cell) {
        cell = [[BigViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
    }
    if (self.courseAry.count > indexPath.row) {
        cell.model = [self.courseAry objectAtIndex:indexPath.row];
