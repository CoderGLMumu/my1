    ...
    for (NSDictionary *dictM in self.imgDataSource) {
        for (NSString *userNameT in dictM.allKeys) {
            if ([userNameT isEqualToString:username]) {
                ...
                [cell.imageView sd_setImageWithURL:[NSURL URLWithString:value] placeholderImage:[UIImage imageNamed:@"HX_img_head"]];
            }
        }
    }
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (NSDictionary *dictM in self.imgDataSource) {
                for (NSString *userNameT in dictM.allKeys) {
                    if ([userNameT isEqualToString:username]) {
                        NSString *value = dictM[userNameT];
                        [remarkView.glImageView sd_setImageWithURL:[NSURL URLWithString:value] placeholderImage:[UIImage imageNamed:@"HX_img_head"]];
                    }
                }
            }
